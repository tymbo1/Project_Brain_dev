#!/usr/bin/env python3
"""
llm_ingest_ling_pass3b.py — Recovery pass for 16 concepts that failed in Pass 3 (LLM parse errors).

Same pipeline as llm_ingest_ling_pass3.py: --commit → --review → --promote
"""
...
# Anchor IDs from Pass 3 original map (all verified in DB)
PASS3B_ANCHORS = {
...
- Both subject and object must be real linguistics concepts (single nouns or short noun phrases, NO generic terms like "thing", "form", "concept").
- Be precise about direction: if A is part of B, write A part_of B.
- Subject must be closely related to "{concept}" — do not drift to distant concepts.
- Return ONLY a valid JSON array, absolutely no explanation, no markdown, no preamble.

Example format:
[
  {{"subject": "affricate", "predicate": "is_a", "object": "consonant", "confidence": 0.98}},
  {{"subject": "affricate", "predicate": "part_of", "object": "phoneme inventory", "confidence": 0.95}}
]
...
what it is distinct_from, how it relates to closely adjacent linguistics concepts.
...
def main():
    ...
    print(f"\nLinguistics Pass 3b (recovery) — {mode}")
    ...
    if not args.dry_run:
        ...
        print(f"\nNext: python3 llm_ingest_ling_pass3b.py --review")
        print(f"      python3 llm_ingest_ling_pass3b.py --promote")
    else:
