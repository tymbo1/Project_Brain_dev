...
from symbolic_mutator import mutate_pending
from identity_matrix import IdentityMatrix
from symbolic_core.symbolic_layers import find_layer
...
from reinforcement_net import ( propagate_bridge_weights,
    propagate_bridge_weights_recursive, apply_bridge_strategy,
)
...
class SelyrionCore:
    ...
    def teach(self, statement: str) -> dict:
        ...
        # Attempt mutation feedback loop
            ...
            for mutated in mutated_terms:
                if symbolic_acceptance(mutated, self.symbolic_memory, self.known_patterns, self.anchors, self.identity_matrix):
                    ...
                    print(f"[🌀] Mutated form accepted: {mutated}")
                else:
                    print(f"[⚠️] Mutation still rejected: {mutated}")
    ...
    def infer(self, intent, terms, symbolic) -> dict:
        ...
        if intent in ["what", "how"]:
            apply_bridge_strategy(categorized, cross_layer_bridge, strategy="recursive", depth_limit=3)
        elif intent in ["why", "who"]:
            apply_bridge_strategy(categorized, cross_layer_bridge, strategy="linear")
        else:
            apply_bridge_strategy(categorized, cross_layer_bridge, strategy="none")
